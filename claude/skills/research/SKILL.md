...
1. **Search Strategy**:
   ...
   - If `useGemini` is `true`: first validate Gemini CLI available: `command -v gemini >/dev/null 2>&1`. If not found, fall back to WebSearch and warn: "Gemini CLI not installed, using WebSearch."
   - If available, execute research from a temp dir to avoid GEMINI.md interception: `cd /tmp && timeout 120 gemini -y -m <gemini.model> --prompt "...your search prompt..." 2>&1`. Check exit code — if non-zero or output contains `GaxiosError` or `RESOURCE_EXHAUSTED`, fall back to WebSearch for that query and warn: "Gemini CLI failed (model capacity or rate limit), falling back to WebSearch." Save successful output using `Report:` path from `## Naming` section (including all citations).
