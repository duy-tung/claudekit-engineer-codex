...
The skill is located at `${CLAUDE_PLUGIN_ROOT}/skills/plans-kanban/`.
...
```bash
node ${CLAUDE_PLUGIN_ROOT}/skills/plans-kanban/scripts/server.cjs --stop
```
...
node ${CLAUDE_PLUGIN_ROOT}/skills/plans-kanban/scripts/server.cjs \
  --dir "$PLANS_DIR" \
  --host 0.0.0.0 \
  --open \
  --foreground
...
```json
{
  "command": "node ${CLAUDE_PLUGIN_ROOT}/skills/plans-kanban/scripts/server.cjs --dir \"./plans\" --host 0.0.0.0 --open --foreground",
